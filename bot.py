...
async def work_with_user(message: types.Message, state: FSMContext):
    try:
        async with state.proxy() as info:
            ...
            osu.search(query=info["user_name"]).users.data[0].id
    except IndexError:
        await message.answer("Вы ввели неправильно никнейм")
        return
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    buttons = ['Посмотреть количество игр','Посмотреть онлайн ли он', 'Посмотреть его аватар','Посмотреть когда он зарегистрировался','Посмотреть на какой карте больше всего попыток','Посмотреть лучший скор','Назад']
    keyboard.add(*buttons)
    await States.next()
...
@dp.message_handler(text = 'Посмотреть когда он зарегистрировался', state = States.info_about_user)    
...
@dp.message_handler(text = 'Посмотреть на какой карте больше всего попыток', state = States.info_about_user)    
async def get_player_beatmap_playcount(message: types.Message, state: FSMContext):
    async with state.proxy() as info:
        user = osu.search(query=info["user_name"]).users.data[0]
    beatmap_title = osu.user_beatmaps(user_id= user.id, type_="most_played")[0].beatmap().beatmapset().title
    beatmap_info_id = osu.user_beatmaps(user_id= user.id, type_="most_played")[0].beatmap().id
    await message.answer(f"""Название карты: {osu.search_beatmapsets(query=beatmap_title).beatmapsets[0].title}
Количество попыток: {osu.user_beatmaps(user_id=user.id, type_="most_played")[0].count}
Дата: {osu.beatmap_user_score(beatmap_id=beatmap_info_id, user_id=user.id).score.created_at} 
Количество PP: {osu.beatmap_user_score(beatmap_id=beatmap_info_id, user_id=user.id).score.pp}
Превью карты: {osu.user_beatmaps(user_id= user.id, type_="most_played")[0].beatmap().beatmapset().preview_url}""")
...
async def get_player_best_score(message: types.Message, state: FSMContext):
    async with state.proxy() as info:
        user = osu.search(query=info["user_name"]).users.data[0]
    beatmap_info = osu.user_scores(user.id, "best")[0]
    beatmap_info_id = beatmap_info.beatmap.id
    beatmap_title = beatmap_info.beatmap.beatmapset().title
    await message.answer("Для получения времени МСК в дате +3 часа")
    await message.answer(f"""Название карты: {beatmap_title}
Дата: {beatmap_info.created_at}
Количество PP: {osu.beatmap_user_score(beatmap_id=beatmap_info_id, user_id=user.id).score.pp}
Место на карте: {osu.beatmap_user_score(beatmap_id=beatmap_info_id, user_id=user.id).position}
Превью карты:{beatmap_info.beatmap.beatmapset().preview_url}""")
...
async def work_with_beatmap(message: types.Message, state: FSMContext):
    try:
        async with state.proxy() as info:
            ...
            osu.search_beatmapsets(query=info["beatmap_name"]).beatmapsets[0]
    except IndexError:
        await message.answer("Вы ввели неправильно название")
        return
    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    buttons = ['Посмотреть количество игр на карте','Посмотреть автора карты','Прослушать превью карты','Дата создания карты','Назад']
    keyboard.add(*buttons)
    await States.next()
    await message.answer("Что Вы хотите узнать?", reply_markup=keyboard)

@dp.message_handler(text = 'Посмотреть количество игр на карте', state = States.info_about_beatmap)
async def get_beatmap_playcount(message: types.Message, state: FSMContext):
    ...
    await message.answer(str(beatmap.play_count))
...
async def get_beatmap_creator(message: types.Message, state: FSMContext):
    async with state.proxy() as info:
        beatmap = osu.search_beatmapsets(query=info["beatmap_name"]).beatmapsets[0]
    await message.answer(beatmap.creator)
...
async def get_beatmap_prewiev(message: types.Message, state: FSMContext):
    async with state.proxy() as info:
        beatmap = osu.search_beatmapsets(query=info["beatmap_name"]).beatmapsets[0]
    await message.answer(beatmap.preview_url)
...
async def get_beatmap_date_creation(message: types.Message, state: FSMContext):
    async with state.proxy() as info:
        beatmap = osu.search_beatmapsets(query=info["beatmap_name"]).beatmapsets[0]
    await message.answer("Для получения времени МСК в дате +3 часа")
    await message.answer(f"Дата создания карты:{str(beatmap.submitted_date)}")    
